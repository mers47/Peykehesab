...
for needle in ['minSdk = 23','targetSdk = 36','isCoreLibraryDesugaringEnabled = true','lifecycle-runtime-compose:2.11.0']:
    if needle not in gradle:fail(f'تنظیم Runtime ناقص: {needle}')
for needle in ['api-level: [23, 35, 36]','connectedDebugAndroidTest','assembleRelease','timeout 30s','uiautomator','monkey','--monitor-native-crashes','ANR in ir\\.peykhesab\\.app','wm size 1600x1200','font_scale 2.0','cmd uimode night yes','user_rotation 1','PRODUCTION_RUNTIME_GATE=PASS']:
...
print('AUDIT_PASS_3_OK lifecycle=1 no_main_thread_db=1 runtime_api_23_35_36=1 release_monkey_anr=1 rotation_large_dark=1')
